...
export function useAuthWs() {
  ...
  useEffect(() => {
    ...
    if (!sessionId || !persoId || !wsMac) {
      console.log(
        "[WS] Prerequisites for WebSocket connection not met (sessionId, persoId, or wsMac missing).",
      );
    ...
    if (wsRef.current) {
      ...
      console.log(
        "[WS] Connection attempt skipped: wsRef.current already exists.",
      );
      return;
    }
    ...
    const open = () => {
      ...
      const httpBase = import.meta.env.VITE_APP_API_URL;
      ...
      if (!currentSessionId || !currentPersoid || !currentWsMac) {
        console.log(
          "[WS] Auth details became invalid before scheduled (re)connect. Aborting open.",
        );
        setWsReady(false);
        return;
      }
      ...
      */
      ...
      ws.onmessage = async (ev) => {
        ...
        if (receivedData === "LOGOUT") {
          console.log(
            "[WS] Received LOGOUT command from server. Initiating full client logout...",
          );
          setWsReady(false); // WS no longer ready
          try {
            useAuthStore
              .getState()
              .setAuthEvent({ type: "server_logout", at: Date.now() });
            await callLogout("silent");
          } catch (logoutError) {
            console.error(
              "[WS] Error during callLogout operation triggered by server LOGOUT:",
              logoutError,
            );
          }
        ...
        if (receivedData === "reauth-fail") {
          console.error(
            "[WS] In-band re-authentication failed by server (reauth-fail). Token may be invalid.",
          );
      ...
      ws.onclose = (ev) => {
        console.log(
          `[WS 🚫 CLOSED] Code: ${ev.code}, Reason: "${ev.reason || "No reason given"}", WasClean: ${ev.wasClean}`,
        );
        wsRef.current = undefined;
        ...
        if (ev.code !== 1000 && ev.code !== 1001) {
          ...
          if (
            ...
            console.log(
              `[WS] Connection closed unexpectedly. Retrying in ${retryDelay / 1000}s...`,
            );
            tRef.current = setTimeout(open, retryDelay);
            backoffRef.current = Math.min(retryDelay * 2, 30_000);
          } else {
            console.log(
              "[WS] Auth details missing after unexpected close. Not retrying.",
            );
            clearTimeout(tRef.current); // Ensure no pending retry if auth details were cleared
          }
        } else {
          console.log(
            "[WS] Connection closed gracefully or intentionally. No retry.",
          );
    ...
    return () => {
      ...
      if (wsRef.current) {
        console.log(
          "[WS] Closing WebSocket due to useEffect cleanup (deps changed or unmount). URL:",
          wsRef.current.url,
        );
