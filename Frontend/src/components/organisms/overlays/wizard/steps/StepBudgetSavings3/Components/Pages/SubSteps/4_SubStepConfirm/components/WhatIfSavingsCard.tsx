...
function futureValueMonthlyPmt(pmt: number, annualRatePct: number, years: number) {
...
export default function WhatIfSavingsCard({ maxMonthly = 50_000, defaultMonthly = 1000 }: Props) {
    const currency = useAppCurrency();
    const locale = useAppLocale();
    const money0 = (v: number) => formatMoneyV2(v ?? 0, currency, locale, { fractionDigits: 0 });
    ...
    return (
      ...
      "
        >
          ...
          "
                ...
                <div className="min-w-0">
                    ...
                    <div className="mt-3 inline-flex flex-wrap items-center gap-2">
                        <span className="rounded-full bg-wizard-surface border border-wizard-stroke/20 px-3 py-1.5 shadow-sm shadow-black/5">
                            <span className="text-xs font-semibold text-wizard-text/60">+ </span>
                            <span className="money font-extrabold text-wizard-accent tabular-nums">
                                {money0(clampedMonthly)}
                            </span>
                            <span className="text-xs font-semibold text-wizard-text/60"> /mån</span>
                        </span>

                        <span className="rounded-full bg-wizard-surface border border-wizard-stroke/20 px-3 py-1.5 shadow-sm shadow-black/5">
                            <span className="text-xs font-semibold text-wizard-text/60">i </span>
                            <span className="font-extrabold text-wizard-text tabular-nums">{years}</span>
                            <span className="text-xs font-semibold text-wizard-text/60"> år</span>
                        </span>

                        <span className="rounded-full bg-wizard-surface border border-wizard-stroke/20 px-3 py-1.5 shadow-sm shadow-black/5">
                            <span className="text-xs font-semibold text-wizard-text/60">med </span>
                            <span className="font-extrabold text-wizard-text tabular-nums">{rate}%</span>
                            <span className="text-xs font-semibold text-wizard-text/60"> avkastning</span>
                        </span>
                    </div>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                <ControlCard
                    title="Per månad"
                    value={<span className="text-darkLimeGreen tabular-nums">{money0(clampedMonthly)}</span>}
                >
                    <Slider value={[clampedMonthly]} min={0} max={maxMonthly} step={100} onValueChange={([v]) => setMonthly(v ?? 0)} />
                ...
                <ControlCard title="År" value={<span className="tabular-nums">{years}</span>}>
                ...
                >
                    <Slider value={[rate]} min={0} max={10} step={0.5} onValueChange={([v]) => setRate(v ?? 0)} />
                    <div className="mt-2 flex items-center justify-between text-[11px] font-semibold text-wizard-text/45">
                        <span>0%</span>
                        <span>10%</span>
                    </div>
        ...
        "
                ...
                <div className="mt-2 flex items-end justify-between gap-4">
                    <div className="min-w-0">
                        <p className="text-sm text-wizard-text/70 leading-snug">
                            Du sätter in{" "}
                            <span className="font-semibold text-wizard-text tabular-nums">{money0(deposit)}</span>{" "}
                            och får{" "}
                        ...
                        <p className="mt-2 text-xs text-wizard-text/55">
                            Totalt värde efter <span className="font-semibold">{years}</span> år
                        </p>
                    </div>
            ...
            "
                    >
                        <span className="money text-lg sm:text-xl font-extrabold text-wizard-text tabular-nums">
                            {money0(result.fv)}
                        </span>
                    </div>
                </div>
