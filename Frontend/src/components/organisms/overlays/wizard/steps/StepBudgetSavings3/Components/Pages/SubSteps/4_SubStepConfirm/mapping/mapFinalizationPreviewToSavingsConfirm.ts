...
import { mapPreviewIncome, type PreviewIncomeVm } from "@/utils/wizardPreview/mapPreviewIncome";
...
export function mapFinalizationPreviewToSavingsConfirm(
    ...
    const goals =
        (s?.goals ?? [])
            .map((g) => {
                ...
                const targetDateLabel = g.targetDate ? formatShortDate(g.targetDate, locale) : undefined;
            ...
            .filter((g) => g.monthlyContribution !== 0 || g.targetAmount || g.amountSaved || g.targetDateLabel)
            .sort(
                (a, b) =>
    ...
    return {
        ...
        goals,
        disposableAfterExpensesMonthly: num(preview.disposableAfterExpensesWithCarryMonthly),
        disposableAfterExpensesAndSavingsMonthly: num(preview.disposableAfterExpensesAndSavingsWithCarryMonthly),
    };
}
