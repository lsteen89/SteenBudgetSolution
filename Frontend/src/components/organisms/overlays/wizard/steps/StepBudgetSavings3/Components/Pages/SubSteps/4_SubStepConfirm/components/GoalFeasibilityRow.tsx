...
export default function GoalFeasibilityRow({
    ...
    const statusLine = useMemo(() => {
        ...
        // Normal case
        if (etaMonths === 1) return "Takt ser bra ut — målet kan nås inom ~1 månad.";
        if (etaMonths != null && etaMonths <= 12) return `Takt ser bra ut — ~${etaMonths} månader kvar.`;
        if (etaMonths != null && etaMonths <= 36) return `Det här är långsiktigt — ~${etaMonths} månader kvar.`;

        return etaMonths != null ? `Det här tar tid med nuvarande takt — ~${etaMonths} månader.` : "—";
    }, [target, remaining, contrib, etaMonths, money0]);

    const tone =
        target > 0 && remaining === 0 ? "done" :
            contrib <= 0 ? "missing" :
                etaMonths != null && etaMonths <= 12 ? "good" :
                    etaMonths != null && etaMonths <= 36 ? "ok" :
                        "slow";
            ...
            >
                ...
                <div className="min-w-0">
                    <div className="flex items-start justify-between gap-2 min-w-0">
                        <div className="min-w-0">
                            <p className="truncate text-sm font-semibold text-wizard-text">{title}</p>
                            ...
                            <span
                                className={cn(
                                    ...
                                    tone === "done" && "border-wizard-accent/20 bg-wizard-accent/10 text-wizard-accent",
                                    tone === "missing" && "border-wizard-warning/25 bg-wizard-warning/10 text-wizard-warning",
                                    tone === "good" && "border-wizard-accent/20 bg-wizard-accent/10 text-wizard-text",
                                    tone === "ok" && "border-wizard-stroke/25 bg-wizard-shell/45 text-wizard-text/80",
                                    tone === "slow" && "border-wizard-stroke/25 bg-wizard-shell/35 text-wizard-text/70"
                                )}
                            >
                                {tone === "done" ? "Klart" : tone === "missing" ? "Saknas" : tone === "good" ? "Bra takt" : tone === "ok" ? "Långsiktigt" : "Tar tid"}
                            </span>
                        </div>
                        ...
                        <div className="shrink-0 sm:hidden">
                            ...
                            >
                                <span className="money tabular-nums font-semibold text-wizard-accent">
                                    {contrib > 0 ? money0(contrib) : "—"}
                                </span>
                                <span className="ml-1 text-xs font-semibold text-wizard-text/60">/mån</span>
                            </div>
                        </div>
                    </div>
                    ...
                    <p className="mt-2 text-sm text-wizard-text/70 leading-snug">{statusLine}</p>
                </div>
