...
import { monthlyPayment, isIncomplete, safeNum } from "@/utils/budget/debtMath";
...
export default function DebtItemAccordion({ index, onRemove }: Props) {
    ...
    const name = useWatch({ control, name: `${base}.name` }) ?? `Skuld ${index + 1}`;
    const type = useWatch({ control, name: `${base}.type` }) as DebtFromForm["type"] | undefined;
    ...
    const title = (
        <div className="min-w-0">
            ...
            <div className="mt-0.5 text-[11px] text-wizard-text/55 sm:hidden">
                {typeLabel(type)} • Rest: <span className="tabular-nums font-semibold text-wizard-text/70">{rest}</span>
            </div>
        </div>
    );
    ...
    const subtitle = (
        <div className="text-xs text-wizard-text/65">
            ...
            <div className="hidden sm:block">
                ...
                Rest: <span className="tabular-nums font-semibold text-wizard-text/75">{rest}</span>
                {type !== "revolving" ? (
                    <>
                        {" "}
                        <span className="text-wizard-text/35">•</span> Ränta:{" "}
                        <span className="tabular-nums font-semibold text-wizard-text/75">{apr}</span>%
                        {" "}
                        <span className="text-wizard-text/35">•</span> Löptid:{" "}
                        <span className="tabular-nums font-semibold text-wizard-text/75">{term}</span> mån
                    </>
                ) : null}
            </div>
        </div>
    ...
    // ✅ Optional: show APR/term as meta line (only when relevant) for better hierarchy
    const meta =
        type !== "revolving" ? (
            <span className="hidden sm:inline text-[11px] text-wizard-text/55">
                Ränta: <span className="tabular-nums font-semibold text-wizard-text/70">{apr}%</span>
                {" • "}
                Löptid: <span className="tabular-nums font-semibold text-wizard-text/70">{term} mån</span>
            </span>
        ) : undefined;
