...
export function mapFinalizationPreviewToExpenseConfirm(
    ...
    const orderedKeys = byCategory.map((c) => asCategoryKey(c.categoryKey ?? c.categoryName));

    if ((itemsByCategory.get("subscription")?.length ?? 0) > 0 && !orderedKeys.includes("subscription")) {
    ...
    const categories = orderedKeys
        .map((key) => {
            const beCat = byCategory.find((c) => asCategoryKey(c.categoryKey ?? c.categoryName) === key);
            ...
            const total =
                ...
                (key === "subscription" ? preview.subscriptions?.totalMonthlyAmount : undefined) ??
                sum(items);
