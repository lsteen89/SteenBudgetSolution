...
export type CoachAction =
  ...
  | { kind: "fix"; title: string; detail: string; actionKey: "expenditure" | "savings" | "debts" };
...
export function mapFinalizationPreviewToFinalSummary(
  ...
  const debtPaymentsFromItems = kr(
    (dto.debt?.debts ?? []).reduce((sum, d) => sum + (d.monthlyPayment ?? 0), 0)
  );
  ...
  const incomeRows: SummaryRow[] = [
    { id: "salary", label: "Lön (netto)", value: dto.income?.netSalaryMonthly ?? 0 },
  ...
  const goalSavingsMonthly = (dto.savings?.goals ?? []).reduce((a, g: any) => a + (g.monthlyContribution ?? 0), 0);

  const savingsRate = income > 0 ? (totalSavings / income) : 0;

  const highestAprDebt = (dto.debt?.debts ?? []).reduce((best: any, d: any) =>
    (d?.apr ?? 0) > (best?.apr ?? 0) ? d : best
    , null);
  ...
  const verdict: FinalSummaryVm["verdict"] =
    verdictKind === "good"
      ? { kind: "good", title: "Tryggt", detail: `Du har ett överskott på ${money0(finalBalance)} per månad. Skapa budgeten — du kan finjustera efteråt.` }
      : verdictKind === "tight"
        ? { kind: "tight", title: "Tight", detail: `Det går ihop, men marginalen är liten. Skapa budgeten och justera vid behov.` }
        : { kind: "bad", title: "Ohållbart", detail: `Du går minus varje månad. Skapa budgeten ändå — men vi markerar vad som bör justeras direkt.` };

  const healthChips: HealthChip[] = [
    {
      label: `Spargrad ${(savingsRate * 100).toFixed(0)}%`,
      tone: savingsRate >= 0.15 ? "good" : savingsRate >= 0.05 ? "neutral" : "warn",
    },
    highestAprDebt?.apr
      ? {
  ...
  const coach: CoachAction =
    verdictKind === "bad"
      ? { kind: "fix", title: "En enkel fix nu", detail: "Sänk sparandet eller justera utgifter så att du går plus.", actionKey: "savings" }
      : verdictKind === "tight"
        ? { kind: "suggest", title: "Gör planen enklare", detail: "Välj ett huvudmål och låt resten vänta tills du har mer marginal." }
        : { kind: "suggest", title: "Bra läge", detail: "Vill du att överskottet går till buffert eller extra amortering?" };
