    ...
    <div className="flex  justify-center items-center mb-6 px-4 w-full">
      ...
      <div className="group flex hover:scale-110 transition-transform duration-300">
        <IconButton
          ...
          IconComponent={(props) => (
            <HomeIcon {...props} className="w-[80%] h-[80%] text-black hover:scale-110 hover:text-darkLimeGreen transition-transform duration-300" />
          )}
      ...
      {!isDesktop && (
        <IconButton
          ...
          iconText="✕"
          className="text-5xl bg-black font-bold focus:outline-none focus:ring-2 focus:ring-white rounded-md w-10 h-10 flex " // 64px x 64px
        />
      )}
    </div>
  );
