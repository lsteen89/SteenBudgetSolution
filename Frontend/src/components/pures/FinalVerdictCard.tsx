...
import { finalVerdictCardDict } from "@/utils/i18n/wizard/stepFinal/FinalVerdictCard.i18n";
...
const FinalVerdictCard: React.FC<FinalVerdictCardProps> = ({
  ...
  const t = <K extends keyof typeof finalVerdictCardDict.sv>(k: K) =>
    tDict(k, locale, finalVerdictCardDict);
  ...
  const badgeText = balance >= 0 ? t("surplusPerMonth") : t("deficitPerMonth");
  ...
  return (
    <WizardCard>
      ...
      <div className="mt-5">
        ...
        <p className="mt-4 text-sm text-wizard-text/65">{t("footer")}</p>
      </div>
    </WizardCard>
  );
};
